...
def dropAutoWrite():
    ...
    # Tweaked proj_root to allow for "Artist Name" in root of Projects Folder - AM 9/23/15
    t = nuke.Tab_Knob("Path Fragments")
    w.addKnob(t)
    w.addKnob(nuke.EvalString_Knob('proj_root', 'Project Root', 'E:/[lrange [split [value root.name] / ] 2 2 ]/[lrange [split [value root.name] / ] 2 2 ]/RENDERS'))
    ...
    w.addKnob(nuke.EvalString_Knob('filename', 'File Name', '[lindex [split [lindex [split [knob [topnode].file] .] 0] /] end]'))
    ...
    output_path = "[value proj_root]/[value script]/[value script].%04d.dpx"
...
m.addSeparator()
m.addCommand('AutoWrite', 'autowrite.dropAutoWrite()','Shift+W',)
